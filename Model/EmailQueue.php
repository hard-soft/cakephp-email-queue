...
App::import('SerializeType', 'Model/Types');
 ...
 * Serialized fields
 */
	public $_serialized = array('template_vars', 'headers', 'attachments', 'email_to', 'email_cc', 'email_bcc');
	...
	public function enqueue($to, array $data, $options = array()) {
		...
		foreach (array('cc', 'bcc') as $k) {
            if (!empty($options[$k])) {
                ...
                $email += array("email_{$k}" => $options[$k]);
            }
        }

		$emails = array();
        // foreach ($to as $t) {
            $emails[] = array('email_to' => $to) + $email;
        // }
 ...
 **/
	public function afterFind($results, $primary = false) {
		...
		foreach ($results as &$r) {
			foreach ($this->_serialized as $f) {
				if (isset($this->data[$this->alias][$f])) {
					$r[$this->alias][$f] = $this->toPHP($this->data[$this->alias][$f]);
				}
			}
		}
